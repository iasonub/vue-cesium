...
var DrawHelper = (function () {
    ...
    _.prototype.startDrawing = function (cleanUp) {
      ...
      //空间两点距离计算函数
    _.prototype.getSpaceDistance= function (positions) {
    let distance = 0;
  ...
  _.prototype.getArea=function(points) {
    var _self = this;
    let res = 0;
    for (let i = 0; i < points.length - 2; i++) {
      let j = (i + 1) % points.length;
      let k = (i + 2) % points.length;
      let totalAngle =_self.Angle(points[i], points[j], points[k]);


      let dis_temp1 = _self.distance(positionsFloat[i], positionsFloat[j]);
      let dis_temp2 = _self.distance(positionsFloat[j], positionsFloat[k]);
  ...
  /**角度 */
  _.prototype.Angle=function(p1, p2, p3) {
      var _self = this;
    let bearing21 = _self.Bearing(p2, p1);
    let bearing23 =_self. Bearing(p2, p3);
  ...
  /**方向 */
_.prototype.Bearing=function(from, to) {
  ...
  _.prototype.distance=function(point1, point2) {
    ...
    _.PolylinePrimitive = (function () {
		...
		_.prototype.update = function (context, frameState, commandList) {
		    ...
		    if (this._createPrimitive ||
		        ...
		        if (this.strokeColor && this.getOutlineGeometry) {
		            // create the highlighting frame
		            this._outlinePolygon = new Cesium.GroundPolylinePrimitive({
		                geometryInstances: new Cesium.GeometryInstance({
		                    geometry: this.getOutlineGeometry(),
		                    attributes: {
		                        color: Cesium.ColorGeometryInstanceAttribute.fromColor(this.strokeColor)
    ...
    var defaultBillboard = {
        iconUrl: require("@/assets/img/dragIcon.png"),
        shiftX: 0,
        shiftY: 0
    }
	...
	_.prototype.startDrawingSector = function (options) {
	   ...
	   // var tooltip = this._tooltip;
	    ...
	    var options = copyOptions(options, defaultSurfaceOptions);
	    var poly = new DrawHelper.PolygonPrimitive(options);
	        ...
	        if (position != null) {
	            if (circle == null) {
	               // tooltip.showAt(position, "<p>Click to add first point</p>");
	                ...
	                var cartesian = scene.camera.pickEllipsoid(position, ellipsoid);
	                if (cartesian) {
	                    var Heading = getHeading(circle,cartesian);
						var d = Heading[0].toFixed(2);
						
						var carto_lt = ellipsoid.cartesianToCartographic(circle);
						var lonlat = {lon:Cesium.Math.toDegrees(carto_lt.longitude),lat:Cesium.Math.toDegrees(carto_lt.latitude)};
						var lonlats = calcSector(lonlat,Heading[1],d,0);
						
						lonlats.push(lonlat.lon,lonlat.lat);
						poly.positions = positions = Cesium.Cartesian3.fromDegreesArray(lonlats);
        ...
        mouseHandler.setInputAction(function (movement) {
            if (movement.position != null) {
                ...
                if (cartesian) {
                    ...
                    positionsFloat=positions
                    // add marker at the new position
                    markers.addBillboard(cartesian);
					if (typeof options.clickCallback == 'function') options.clickCallback(cartesian);
                }
            }
            //在三维场景中添加点
                ...
                if (positions.length == 0) {
                   // tooltip.showAt(position, "<p>Click to add first point</p>");
                    ...
                    if (cartesian) {
						...
						if (typeof options.moveCallback == 'function') options.moveCallback(cartesian);
                        // show tooltip
                ...
                if (positions.length < minPoints + 2) {
                    ...
                    if (cartesian) {
                        if (typeof options.endCallback == 'function') {
                            ...
                            var item={distance,area}
                            options.endCallback(positions,item);
                        }
                        _self.stopDrawing();
                    }
                }
            ...
            if (movement.position != null) {
                ...
                if (cartesian) {
                    if (extent == null) {
                        ...
                        _self.stopDrawing();
                        if (typeof options.endCallback == 'function') {
                            options.endCallback(getExtent(firstPoint, ellipsoid.cartesianToCartographic(cartesian)));
                        }
                    }
                }
            }
            ...
            if (movement.position != null) {
                ...
                if (cartesian) {
                    if (circle == null) {
                        ...
                        if (typeof options.endCallback == 'function') {
                            options.endCallback(circle.getCenter(), circle.getRadius());
                        }
                        _self.stopDrawing();
                    }
                }
            ...
            if (position != null) {
                if (circle == null) {
                    ...
                    if (cartesian) {
						...
						if (typeof options.moveCallback == 'function') options.moveCallback({position:cartesian,radius:circle.getRadius(),center:circle.getCenter()});
	...
	function calcSector(p,r,angle){
		angle = angle*1;
	    //var earthRadius = 6378137;  //长轴
		//var earthRadius = 6356752.314;   //短轴
	    //var earthRadius = 5192038.0;   2000坐标系
	    var returnCoords = [];
	    var mapRadiusx = Math.abs(r / 6378137 * 180.0 / Math.PI);
	    var mapRadiusy = Math.abs(r / 6378137 * 180.0 / Math.PI);
	    for (var i = 0; i < (angle-1); i++){
	        var mapX = p.lon + mapRadiusx * Math.cos(Cesium.Math.toRadians(i+270));
	        var mapY = p.lat + mapRadiusy * Math.sin(Cesium.Math.toRadians(i+90));
	        returnCoords.push(mapX,mapY);
	    }
	    return returnCoords;
	}
	
	function getHeading(start,end,enter) {
		enter = enter||1;
		...
		var HeadingInMeters = Cesium.Math.toDegrees(geodesic.endHeading);
		if(HeadingInMeters < 0) HeadingInMeters += 360;
		var lengthInMeters = Math.round(geodesic.surfaceDistance);
		lengthInMeters = (lengthInMeters / enter).toFixed(3)*1
		return [HeadingInMeters,lengthInMeters];
	}
